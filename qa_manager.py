...
class Training_Manager():
    ...
    def start_qa(self, metrics, params):
        self.trainer.qa(metrics = metrics, params = params)

    def qa(self, metrics, params):
        for i in range(params.len_param_combinations()):
            params.set_param_combination(i)
            dataloaders = data_loader.fetch_dataloader(
                    ['val', 'test'], batch_size = params.batch_size, params = params)
            train_dl = None
...
if __name__ == '__main__':
    ...
    manager.qa(metrics = metrics, params = params)
